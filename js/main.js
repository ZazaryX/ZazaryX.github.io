/* zazary site - main bundle
   ========================================================================
   single runtime file, no framework. runs as a deferred classic script so
   both hosted and direct-disk previews work. structure (top-down):
...
const I18N_BUNDLE = 'js/i18n.bundle.min.js?v=20260917j';
...
const state = {
    ...
    heroVideoObjectUrls: [],
    mobileHeroReady: false,
    mobileHeroPromise: null,
    portfolioImagePromises: [],
    portfolioImagesReady: Promise.resolve([]),
    portfolioVideoPromises: [],
    portfolioVideosReady: Promise.resolve([])
};
...
*/
let i18nBundlePromise = null;
function loadI18nBundle() {
    if (globalThis.ZAZARY_I18N) return Promise.resolve(globalThis.ZAZARY_I18N);
    if (!i18nBundlePromise) {
        i18nBundlePromise = new Promise((resolve, reject) => {
            const script = document.createElement('script');
            script.src = I18N_BUNDLE;
            script.defer = true;
            script.onload = () => globalThis.ZAZARY_I18N
                ? resolve(globalThis.ZAZARY_I18N)
                : reject(new Error('bundled i18n data is empty'));
            script.onerror = () => reject(new Error('bundled i18n data failed to load'));
...
async function loadI18n(lang) {
    ...
    // Browsers intentionally block fetch() for file:// JSON. The compact
    // local bundle keeps language switching and portfolio dialogs functional
    // when index.html is opened directly from an extracted Windows folder.
    if (location.protocol === 'file:') {
        const bundle = await loadI18nBundle();
        if (!bundle[lang]) throw new Error('bundled i18n language is missing: ' + lang);
        return bundle[lang];
    }
    const v = '20260917j';
    try {
        ...
        // Also recover if a static host serves JSON with an incorrect MIME
        // type or blocks it through an over-restrictive rule.
        const bundle = await loadI18nBundle();
        if (!bundle[lang]) throw error;
        return bundle[lang];
    }
}
...
function openProject(id) {
    ...
    const published = $('#folioModalPublished');
    const publishedRow = published?.closest('.folio-published');
    if (published && rec.published) {
        published.textContent = rec.published;
        if (rec.publishedISO) published.setAttribute('datetime', rec.publishedISO);
        else published.removeAttribute('datetime');
        if (publishedRow) publishedRow.hidden = false;
    } else if (publishedRow) {
        publishedRow.hidden = true;
    }
    ...
    const hideMedia = () => {
        ...
        if (img) { img.hidden = true; img.classList.remove('is-loaded'); img.removeAttribute('src'); img.removeAttribute('alt'); }
    };
    hideMedia();
    ...
    /* Optional process case. Only Makima has a recording; every other live
       project gets an explicit localised note instead of a fake placeholder. */
    const caseRoot = $('#folioCase');
    const caseVideo = $('#folioCaseVideo');
    const caseNote = $('#folioCaseNote');
    if (caseVideo) {
        try { caseVideo.pause(); } catch (_) {}
        caseVideo.replaceChildren();
        caseVideo.removeAttribute('poster');
        caseVideo.removeAttribute('aria-label');
        try { caseVideo.load(); } catch (_) {}
        caseVideo.hidden = true;
    }
    if (caseRoot) caseRoot.hidden = false;
    if (rec.caseVideo && caseVideo) {
        if (caseNote) caseNote.hidden = true;
        caseVideo.hidden = false;
        if (rec.casePoster) caseVideo.poster = rec.casePoster;
        caseVideo.setAttribute('aria-label', t.modal?.casePlay || `${rec.title || id} process case`);
        caseVideo.appendChild(el('source', { src: rec.caseVideo, type: 'video/webm' }));
        if (rec.caseVideoFallback) {
            caseVideo.appendChild(el('source', { src: rec.caseVideoFallback, type: 'video/mp4' }));
        }
        caseVideo.load();
        caseVideo.play().catch(() => {});
    } else if (caseNote) {
        caseNote.textContent = t.modal?.caseNotRecorded || 'The creation process was not recorded for this project.';
...
/* ---------- Preloader ---------- */
function waitForDecodedHeroImage(image, timeoutMs = 4500) {
    return new Promise((resolve) => {
        ...
        const finish = (ok) => {
            ...
            // `load`/naturalWidth already prove that the selected resource is
            // usable. Awaiting decode() here delayed mobile LCP by seconds on
            // throttled CPUs even though the poster was ready to paint.
        ...
        timer = setTimeout(() => finish(image.complete && image.naturalWidth > 0), timeoutMs);
        if (image.complete) queueMicrotask(() => finish(image.naturalWidth > 0));
    });
}

function initPreloader() {
    // Original Zazary presentation plus a real all-page readiness gate. The
    // bar is visually simulated for smoothness, while release is controlled by
    // concrete checks for CSS, locale/render, fonts, icons, hero media and all
    // eight published portfolio previews. Full modal-only media remains lazy.
    ...
    if (status) status.textContent = status.textContent.trim() || 'loading';

    const withTimeout = (promise, timeoutMs, fallback = false) => Promise.race([
        Promise.resolve(promise),
        new Promise((resolve) => setTimeout(() => resolve(fallback), timeoutMs))
    ]);

    const cssLink = $('link[href*="style.min.css"]');
    const cssTask = new Promise((resolve) => {
        ...
        const timer = setTimeout(() => done(applied()), 10000);
        cssLink.addEventListener('load', () => requestAnimationFrame(() => done(true)), { once: true });
        cssLink.addEventListener('error', () => done(false), { once: true });
    });

    const localeTask = withTimeout(ensureInitialI18n().then(Boolean), 12000, false);

    const fontsTask = localeTask.then(async () => {
        if (!document.fonts?.load) return true;
        const fonts = [
            ['400 16px Rajdhani', 'portfolio'],
            ['500 16px Rajdhani', 'portfolio'],
            ['600 16px Rajdhani', 'portfolio'],
            ['700 16px Rajdhani', 'portfolio'],
            ['700 16px Orbitron', 'ZAZARY'],
            ['700 32px "Noto Sans JP"', '鬼']
        ];
        if (state.lang === 'ja') fonts.push(['500 16px "Noto Sans JP Locale"', '作品']);
        if (state.lang === 'zh') fonts.push(['500 16px "Noto Sans SC"', '作品']);
        const result = await withTimeout(Promise.allSettled(
            fonts.map(([spec, sample]) => document.fonts.load(spec, sample))
        ), 10000, null);
        if (!result) return false;
        await withTimeout(document.fonts.ready, 3000, null);
        return result.every((entry) => entry.status === 'fulfilled' && entry.value.length > 0);
    });

    const iconTask = (() => {
        const url = 'images/icons.svg?v=20260917j';
        if (location.protocol === 'http:' || location.protocol === 'https:') {
            return withTimeout(fetch(url, { cache: 'force-cache' })
                .then((response) => response.ok), 10000, false);
        }
        return new Promise((resolve) => {
    ...
    const posterTask = (async () => {
        let ok = await waitForDecodedHeroImage(posterImage, 8000);
        if (!ok && posterImage) {
            posterImage.closest('picture')?.querySelectorAll('source').forEach((source) => source.remove());
            posterImage.src = 'images/hero-static-desktop.webp?v=20260917j';
            ok = await waitForDecodedHeroImage(posterImage, 5000);
        }
        posterOk = ok;
        return ok;
    })();

    const pageLoadTask = new Promise((resolve) => {
    ...
    const rawTasks = [
        ['stylesheet', cssTask],
        ['locale-and-render', localeTask],
        ['fonts', fontsTask],
        ['icon-sprite', iconTask],
        ['hero-poster', posterTask],
        ['window-load', pageLoadTask]
    ];

    state.portfolioImagePromises.forEach((task, index) => {
        rawTasks.push([`portfolio-image-${index + 1}`, task]);
    });
    state.portfolioVideoPromises.forEach((task, index) => {
        rawTasks.push([`portfolio-video-${index + 1}`, task]);
    });

    if (matchMedia('(max-width: 768px)').matches) {
        rawTasks.push(['hero-mobile-motion', prepareMobileHeroVideo($('#heroMobileVideo'))]);
    } else if (!reducedMotion() && !navigator.connection?.saveData) {
        $$('.bg-video').forEach((video, index) => {
            rawTasks.push([`hero-desktop-layer-${index + 1}`, prepareHeroVideo(video)]);
        });
    }

    const trackedTasks = rawTasks.map(([name, task]) => {
        taskState.set(name, 'pending');
        return Promise.resolve(task)
            .then((value) => {
                const ok = value !== false && value != null;
                taskState.set(name, ok ? 'loaded' : 'failed');
                if (!ok) failures.push(name);
                return { name, ok };
            })
            .catch(() => {
                taskState.set(name, 'failed');
                failures.push(name);
    ...
    // Smoothly blend an initial cinematic simulation with actual settled-task
    // progress. It never displays a numeric counter and never reaches 100%
    // until every check has settled or the finite safety limit is reached.
    const animateProgress = (now) => {
        const elapsed = Math.max(0, now - started);
        const phase = Math.min(1, elapsed / SIMULATION_MS);
        const simulated = 6 + (1 - Math.pow(1 - phase, 2.35)) * 70;
        const realRatio = trackedTasks.length ? settledCount / trackedTasks.length : 1;
        const verified = 6 + realRatio * 86;
        const target = Math.min(94, Math.max(simulated, verified));
        const dt = Math.min(0.05, Math.max(0.001, (now - lastFrame) / 1000));
        lastFrame = now;
        progressValue += (target - progressValue) * (1 - Math.exp(-7 * dt));
        if (progress) {
            progress.style.width = `${progressValue.toFixed(2)}%`;
            progress.setAttribute('aria-valuenow', String(Math.round(progressValue)));
        }
        if (!finishing) progressFrame = requestAnimationFrame(animateProgress);
    };
    progressFrame = requestAnimationFrame(animateProgress);

    const allChecks = Promise.all(trackedTasks).then((results) => ({ timedOut: false, results }));
    const hardLimit = new Promise((resolve) => setTimeout(
        () => resolve({ timedOut: true, results: [] }),
        HARD_LIMIT_MS
    ));

    state.heroReady = Promise.race([allChecks, hardLimit])
        .then(async ({ timedOut }) => {
            const remaining = Math.max(0, SIMULATION_MS - (performance.now() - started));
            if (remaining) await new Promise((resolve) => setTimeout(resolve, remaining));
            await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));

            const pending = [...taskState.entries()]
                .filter(([, value]) => value === 'pending')
                .map(([name]) => name);
            const partial = timedOut || failures.length > 0 || pending.length > 0;
            document.body.dataset.preloadState = partial ? 'partial' : 'complete';
            document.body.classList.toggle('preload-partial', partial);
            if (partial) console.warn('Preloader released with fallback:', [...new Set([...failures, ...pending])].join(', '));
            ...
            document.body.classList.toggle('hero-poster-fallback', !posterOk);
        ...
        .catch(() => {
            ...
            document.body.dataset.preloadState = 'partial';
            document.body.classList.add('preload-partial', 'hero-ready', 'hero-poster-fallback');
            document.body.dataset.state = 'ready';
            preloaderEl.classList.add('is-done');
            preloaderEl.setAttribute('aria-busy', 'false');
            setTimeout(() => preloaderEl.remove(), 360);
...
function prepareHeroVideo(video) {
    ...
    if (video._zazaryPreparePromise) return video._zazaryPreparePromise;
    if (video.readyState >= 2 && video.currentSrc) return Promise.resolve(true);

    video._zazaryPreparePromise = new Promise((resolve) => {
        const source = video.querySelector('source');
        const sourceUrl = source?.dataset.src || source?.getAttribute('src');
        if (!source || !sourceUrl) return resolve(false);
        let settled = false;
        const done = (ok) => {
            if (settled) return;
            settled = true;
            ...
            resolve(Boolean(ok && video.readyState >= 2));
        };
        const onReady = () => done(true);
        const onError = () => done(false);
        const timer = setTimeout(() => done(video.readyState >= 2), 25000);
        ...
        if (location.protocol === 'http:' || location.protocol === 'https:') {
            // Fetch every desktop layer completely before attachment. Blob URLs
            // make "hero loaded" a real full-download check, not just metadata
            // or first-frame readiness, and eliminate later range requests.
            fetch(sourceUrl, { cache: 'force-cache' })
                .then((response) => {
                    if (!response.ok) throw new Error('desktop hero fetch failed');
                    return response.blob();
                })
                .then((blob) => {
                    const objectUrl = URL.createObjectURL(blob);
                    state.heroVideoObjectUrls.push(objectUrl);
                    attach(objectUrl);
                })
                .catch(() => attach(sourceUrl));
        } else {
            attach(sourceUrl);
        }
    });
    return video._zazaryPreparePromise;
}

function prepareMobileHeroVideo(video) {
    if (!video || !matchMedia('(max-width: 768px)').matches) return Promise.resolve(true);
    if (state.mobileHeroPromise) return state.mobileHeroPromise;
    state.mobileHeroPromise = new Promise((resolve) => {
        const source = video.querySelector('source');
        ...
        const url = source.dataset.src || source.getAttribute('src');
        const attach = (src) => {
            source.removeAttribute('data-src');
            source.removeAttribute('src');
            video.src = src;
        ...
        if (location.protocol === 'http:' || location.protocol === 'https:') {
            // Download the complete compact MP4 before attaching it. Besides
            // making the progress bar truthful, the resulting Blob prevents a
            // looping media range request from keeping network-idle detectors
            // (and some mobile webviews) open indefinitely.
            fetch(url, { cache: 'force-cache' })
                .then((response) => {
                    if (!response.ok) throw new Error('mobile hero fetch failed');
                    return response.blob();
                })
                .then((blob) => {
                    state.mobileHeroObjectUrl = URL.createObjectURL(blob);
                    attach(state.mobileHeroObjectUrl);
                })
                .catch(() => attach(url));
        } else {
            attach(url);
    ...
    return state.mobileHeroPromise;
}

function playMobileHeroVideo() {
    const video = $('#heroMobileVideo');
    if (!video || !matchMedia('(max-width: 768px)').matches || document.hidden) {
    ...
    const start = () => {
        ...
        if (!result || typeof result.then !== 'function') return Promise.resolve(!video.paused);
        return result.then(() => true).catch(() => false);
    };
    if (video.readyState >= 2) return start();
    return prepareMobileHeroVideo(video).then((ready) => ready ? start() : false);
}

function initMobileHeroVideo() {
    const video = $('#heroMobileVideo');
    if (!video || !matchMedia('(max-width: 768px)').matches) return;
    const reveal = () => {
        if (!video.paused && video.readyState >= 2) {
            state.mobileHeroReady = true;
            document.body.classList.add('hero-mobile-video-ready');
        }
    };
    const attempt = () => playMobileHeroVideo().then((playing) => {
        if (playing) {
            document.body.classList.add('hero-mobile-video-ready');
    ...
    // iOS Low Power Mode and some embedded browsers reject autoplay until a
    // real gesture. Retrying synchronously on the first interaction fixes it.
    window.addEventListener('pointerdown', attempt, { passive: true });
    window.addEventListener('touchend', attempt, { passive: true });
    window.addEventListener('click', attempt, { passive: true });
    prepareMobileHeroVideo(video).then(attempt);
}

function activateHeroVideos() {
    const desktopMotion = matchMedia('(min-width: 769px)').matches;
    if (!desktopMotion || reducedMotion() || navigator.connection?.saveData || state.optimized || state.hidden) {
        return Promise.resolve(false);
    }
...
function initHeroVideoSources() {
    // A complete static <picture> is shown first. Motion is desktop-only: this
    // avoids unsupported VP9 alpha and a 6+ MB transfer on phones and tablets.
    if (!matchMedia('(min-width: 769px)').matches || reducedMotion() || navigator.connection?.saveData) return;
    ...
    // The preloader has already prepared all three layers, so start motion as
    // soon as the loading screen releases instead of leaving a static pause.
    Promise.resolve(state.heroReady).then(startAll);
}
...
/* ---------- Eager portfolio media ---------- */
function initPortfolioImages() {
    const images = $$('.folio-photo[data-src]');
    const activate = (image) => new Promise((resolve) => {
        const url = image.dataset.src;
        if (!url) return resolve(image.complete && image.naturalWidth > 0);
        ...
        const finish = (ok) => {
            ...
            image.removeEventListener('load', onLoad);
            image.removeEventListener('error', onError);
            if (ok && image.naturalWidth) image.classList.replace('is-loading', 'is-loaded');
            else image.classList.remove('is-loading');
            resolve(Boolean(ok && image.naturalWidth));
        };
        const onLoad = () => finish(true);
        const onError = () => finish(false);
        image.addEventListener('load', onLoad, { once: true });
        image.addEventListener('error', onError, { once: true });
        const timer = setTimeout(() => finish(image.complete && image.naturalWidth > 0), 15000);
    ...
    state.portfolioImagePromises = images.map(activate);
    state.portfolioImagesReady = Promise.all(state.portfolioImagePromises);
    return state.portfolioImagesReady;
}

function tuneVideoPlayback() {
    // Download each published portfolio video in full before the preloader can
    // finish. HTTP(S) uses a Blob URL so looping range requests cannot keep the
    // page half-loaded; direct file:// launch uses the local source normally.
    const videos = $$('.folio-video');
    const narrow = window.matchMedia('(max-width: 640px)').matches;
    const saveData = navigator.connection?.saveData;
    const mayAnimate = !narrow && !saveData && !reducedMotion();
    state.portfolioVideoObjectUrls ||= [];

    const activate = (video) => new Promise((resolve) => {
        const source = $('source[data-src], source[src]', video);
        const sourceUrl = source?.dataset.src || source?.getAttribute('src');
        const posterUrl = video.dataset.poster || video.poster;
        if (posterUrl) {
        ...
        const onError = () => finish(false);
        const timer = setTimeout(() => finish(video.readyState >= 2), 20000);
        video.addEventListener('loadeddata', onReady, { once: true });
        video.addEventListener('error', onError, { once: true });
        ...
        if (location.protocol === 'http:' || location.protocol === 'https:') {
            fetch(sourceUrl, { cache: 'force-cache' })
                .then((response) => {
                    if (!response.ok) throw new Error('portfolio video fetch failed');
                    return response.blob();
                })
                .then((blob) => {
                    const objectUrl = URL.createObjectURL(blob);
                    state.portfolioVideoObjectUrls.push(objectUrl);
                    attach(objectUrl);
                })
                .catch(() => attach(sourceUrl));
        } else {
    ...
    state.portfolioVideoPromises = videos.map(activate);
    state.portfolioVideosReady = Promise.all(state.portfolioVideoPromises);

    // Playback remains viewport-aware to avoid burning CPU after everything is
    // downloaded. Fast scrolling only changes play/pause; it never starts a new
    // network request or reveals an empty card.
    if ('IntersectionObserver' in window) {
        const io = new IntersectionObserver((entries) => {
            for (const entry of entries) {
                const video = entry.target;
                if (entry.isIntersecting && mayAnimate) video.play().catch(() => {});
                else video.pause();
            }
        }, { rootMargin: '320px 0px', threshold: 0.01 });
        videos.forEach((video) => io.observe(video));
    } else if (mayAnimate) {
        state.portfolioVideosReady.then(() => videos.forEach((video) => video.play().catch(() => {})));
    }

    return state.portfolioVideosReady;
}
...
async function boot() {
    ...
    initMobileHeroVideo();
    initCardGlow();
    // Start every published preview before creating the preloader gate so its
    // completion check can include each real image/video request.
    initPortfolioImages();
    tuneVideoPlayback();
    initPreloader();
    ...
    if (location.protocol === 'file:') {
        // Direct-disk preview has no HTTP round trip and needs the bundled
        // project records immediately so every portfolio dialog can open.
        hydrate();
    ...
    document.addEventListener('visibilitychange', () => {
        ...
        if (state.hidden) {
            ...
            playVideos();
            playMobileHeroVideo().then((playing) => {
                if (playing) document.body.classList.add('hero-mobile-video-ready');
            });
            scheduleMotionFrame();
        }
    });
